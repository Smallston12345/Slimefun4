...
class MigratorUtil {
    protected static boolean createDirBackup(File dir) {
        try {
            ...
            if (Files.exists(backupPath, LinkOption.NOFOLLOW_LINKS)) {
                Slimefun.logger().log(Level.WARNING, "檢測到已存在的備份數據, 跳過備份");
                return true;
            }
                ...
                try (var fs = Files.walk(src).filter(path -> !Files.isDirectory(path))) {
                    fs.forEach(path -> {
                        ...
                        try {
                            ...
                            Slimefun.logger().log(Level.WARNING, "備份舊數據 " + dir.getName() + " 時出現問題", e);
                        }
                    });
                }
            }
    ...
    protected static void deleteOldFolder(File dir) {
        try {
            ...
            Slimefun.logger().log(Level.WARNING, "刪除文件夾 " + dir.getAbsolutePath() + " 時出現問題", e);
        }
    }
