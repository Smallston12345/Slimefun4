    ...
    public void init() {
        ...
        try {
            ...
            if (readExecutorThread + writeExecutorThread > connectionPoolSize) {
                plugin.getLogger().log(Level.WARNING, "檢測到 block-storage 連接池大小配置小於讀寫線程總和, 可能會導致性能問題");
            }
            ...
            if (blockStorageConfig.getBoolean("delayedWriting.enable")) {
                plugin.getLogger().log(Level.INFO, "已啓用延時寫入功能");
            ...
            plugin.getLogger().log(Level.SEVERE, "加載 Slimefun 方塊存儲適配器失敗", e);
            return;
        }
        ...
        try {
            ...
            if (readExecutorThread + writeExecutorThread > connectionPoolSize) {
                plugin.getLogger().log(Level.WARNING, "檢測到 profile-storage 連接池大小配置小於讀寫線程總和, 可能會導致性能問題");
            }
            ...
            plugin.getLogger().log(Level.SEVERE, "加載玩家檔案適配器失敗", e);
        }
    }
