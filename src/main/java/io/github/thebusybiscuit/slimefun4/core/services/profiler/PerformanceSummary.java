...
class PerformanceSummary {
    ...
    public void send(@Nonnull PerformanceInspector sender) {
        ...
        sender.sendMessage(
                ChatColor.GOLD + "Tick 總用時: " + ChatColor.YELLOW + NumberUtils.getAsMillis(totalElapsedTime));
        sender.sendMessage(ChatColor.GOLD
                ...
                + tickRate
                + " ticks)");
        sender.sendMessage(ChatColor.GOLD + "性能評分: " + getPerformanceRating());
        sender.sendMessage("");
        ...
        summarizeTimings(totalTickedBlocks, "block", sender, items, entry -> {
            ...
            if (sender.getOrderType() == SummaryOrderType.AVERAGE) {
                return String.format(message, average + " | 總用時: " + time);
            } else {
                return String.format(message, time + " | 平均用時: " + average);
            }
        });
    ...
    private TextComponent summarizeAsTextComponent(
        ...
        if (count > 0) {
            TextComponent hoverComponent = new TextComponent("  (將鼠標放置到此處獲取更多信息)");
