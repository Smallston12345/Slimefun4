...
public abstract class AbstractEntityAssembler<T extends Entity> extends SimpleSlimefunItem<BlockTicker>
    ...
    private void updateBlockInventory(BlockMenu menu, Block b) {
        ...
        menu.replaceExistingItem(
                31,
                new CustomItemStack(
                        Material.PISTON, "&7生成高度: &3" + offset + " 格方塊高", "", "&f左鍵: &7+0.1", "&f右鍵: &7-0.1"));
    ...
    protected void constructMenu(BlockMenuPreset preset) {
        preset.addItem(
                1,
                new CustomItemStack(getHead(), "&7在此處放入頭顱", "", "&f此處可以放入作為生成實體頭顱的物品"),
                ChestMenuUtils.getEmptyClickHandler());
        preset.addItem(
                7,
                new CustomItemStack(getBody(), "&7在此處放入組裝原料", "", "&f此處可以放入作為生成實體軀幹的物品"),
                ChestMenuUtils.getEmptyClickHandler());
        preset.addItem(
                13,
                new CustomItemStack(Material.CLOCK, "&7冷卻時間: &b30 秒", "", "&f這個機器需要半分鐘的時間裝配", "&f所以耐心等等吧!"),
                ChestMenuUtils.getEmptyClickHandler());
    }
